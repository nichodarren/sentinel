...
| **4** ✅ | Active Testing | Population-level "recommend next test" | RQ4 → **`notebook.ipynb` §5.5 DONE** (VoI ranking, enriched per-cost + per-subgroup/site; wbc best malaria-adjusted dengue info 0.058; no cheap test resolves dengue) |
| **5** ✅ | Equity *(audit)* + **bio-inspired (ALL-OUT, not committed-minimal)** | Subgroup audit (§5.6 done) + NSGA-II minimal-questionnaire → **`notebook.ipynb` §5.9 DONE** (3-obj dengue-AP/len/site-gap, surrogate-in-search + full-ECC re-score, hypervolume + Jaccard + per-symptom freq + site-robustness + benchmark vs L1/MI/RFE; NSGA-II wins AP 0.450/gap 0.095; stability 0.18 honest) | RQ5 + Primary-Focus "bio-inspired" (pillar now filled) |
...
**Status (2026-06-15):** Phases 0, 1, 1+ (deep-EDA), **2 (Core Modeling RQ1+RQ2), 3 (Safety/conformal RQ3), 4 (Active testing RQ4), 5 (Equity + NSGA-II bio-inspired RQ5), 6 (Global framing) COMPLETE** — `notebook.ipynb` §1–§6 written + executed (**156 cells, 65 code, 0 err, 0 em/en dash, EXIT=0**). RQ4 = §5.5 (VoI), RQ5 bio-inspired = §5.9 (NSGA-II ALL-OUT). **Immediate next step:** **§6 Deployment T0** interactive `ipywidgets` prototype (§6.1/§6.2 currently prose only). Still open (user-owned): **Technical Report PDF 40% + Statement of Originality**.
